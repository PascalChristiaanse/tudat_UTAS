...
.. code-block:: python
    ...
    for i in range(num_observable_types):
        print(i, model_settings.ObservableType(i).name)
            ...
            .value( "one_way_frequency_of_arrival_type", tom::ObservableType::one_way_frequency_of_arrival )
...
m.def( "one_way_frequency_of_arrival",
           &tom::frequencyOfArrivalObservationSettings,
           py::arg( "link_ends" ),
           py::arg( "light_time_correction_settings" ) = std::vector< std::shared_ptr< tom::LightTimeCorrectionSettings > >( ),
           py::arg( "frequency_difference_time_scale" ) = tba::tdb_scale,
           py::arg( "bias_settings" ) = nullptr,
 ...
 Function for creating settings for a frequency of arrival (FOA) observation model.

 Function for creating settings for a frequency of arrival observation model. This observable computes the
 received frequency :math:`f` at a receiver for a transmitted signal, which is sensitive to
 the relative velocity of the transmitter with respect to the receiver.
 For a signal transmitted at frequency :math:`f_{T}` and Doppler ratio :math:`d`,
 the observable :math:`h` is computed from the full relativistic Doppler formula:
 ...
 The transmitter frequency :math:`f_{T}` is obtained from a frequency interpolator set on the transmitting body's vehicle systems.
 If no interpolator is set, a runtime error is raised.
